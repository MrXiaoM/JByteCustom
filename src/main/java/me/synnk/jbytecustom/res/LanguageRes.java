package me.synnk.jbytecustom.res;
...
public class LanguageRes {
        ...
        for (String translation : map.values()) {
            for (char c : translation.toCharArray()) {
                if (!WebLookAndFeel.globalControlFont.canDisplay(c)) {
                    ...
                    JByteCustom.LOGGER.log("Updated WebLaF fonts for unicode support");
                    return;
                }
            }
        }
        JByteCustom.LOGGER.log("Unicode check finished!");
    }
    ...
    private void readXML(Map<String, String> m, InputStream is) {
        try {
            ...
            JByteCustom.LOGGER.err("Failed to load resources: " + e.getMessage());
            e.printStackTrace();
            new ErrorDisplay(e);
        }
    }
